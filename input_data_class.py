...
class inputData_system:
    ...
    # -----------------------------------------
    # Method to load input data - Linear Power Flow   
    def load_input_data_DC(self, _inputDir, _output):
        
        # Opening file
        name_file = self.simulation_set.system_assessment[0] +'_'+self.simulation_set.region_assessment[0] + ".dat"
        ...
        while(True):
            ...
            bus.PL = float(linesfile[cont_line][22:29])        # Load - Active power (MW)
            bus.PL_current = bus.PL
            peak_load += bus.PL
            bus.PG_desp = float(linesfile[cont_line][30:37])   # Base dispatch (MW)
            bus.PG_desp_current = bus.PG_desp
            if(self.simulation_set.version_assessment[0] == 'MRTS-L'):
                # Increases the load by load_factor
                load_factor = 1.00
                bus.PL = load_factor * bus.PL
                bus.PL_current = bus.PL
                bus.PG_desp = load_factor * bus.PG_desp
                bus.PG_desp_current = bus.PG_desp
            bus.area = int(linesfile[cont_line][38:42])        # Area
            bus.int_cost = float(linesfile[cont_line][43:50])  # Interruption cost ($/MW)
            bus.region_interest = int(linesfile[cont_line][51:57])    # Does it participate in the area of ​​interest for reliability assessment? (1: yes; 0: no)
            bus.pos = cont_bus
            if(bus.type == "SW"):
                bus_sw = bus
            cont_bus += 1
        ...
        while(True):
            ...
            cir.cap_n = float(linesfile[cont_line][31:38])     # Normal capacity (MVA)
            cir.cap_e = float(linesfile[cont_line][39:46])     # Emergence capacity (MVA)
            if(self.simulation_set.version_assessment[0] == 'MRTS-L'):
                # Increases the circuit capacity by cap_factor
                cap_factor = 1.00
                cir.cap_n *= cap_factor
                cir.cap_e *= cap_factor
            elif(self.simulation_set.version_assessment[0] == 'MRTS-T' and cir.cap_n != 9999.0):
                # Decreases the circuit capacity by cap_factor
                cap_factor = 1.00
                cir.cap_n *= cap_factor
                cir.cap_e *= cap_factor
            cir.shift_def = float(linesfile[cont_line][47:52]) # Shift transform angle (°)
            cir.area = int(linesfile[cont_line][53:57])        # Area
            cir.pos = cont_cir
            cont_cir += 1
                ...
                if(c.bT.number == int(linesfile[cont_line][0:5])):
                    if(c.bF.number == int(linesfile[cont_line][6:11])):
                        if(c.visited == False):
                            ...
                            break
            if(self.simulation_set.version_assessment[0] == 'MRTS-L'):
                # Decreases the circuit failure rate by failures_rate_factor
                failures_rate_factor = 1.00
                cir_aux.failure_rate = float(linesfile[cont_line][17:27]) / failures_rate_factor
            else:
        ...
        while(True):
            ...
            if(self.simulation_set.version_assessment[0] == 'MRTS-L'):
                # Increases the generating unit capacity by gen_factor
                gen_factor = 1.00
                gstat.P_max = gen_factor * gstat.P_max         # Maximum capacity (MW)
                gstat.P_max_orig = gstat.P_max
            gstat.cost = float(linesfile[cont_line][52:60])    # Cost ($/MWh)
            gstat.pos = cont_gstat
            bus_aux.gstat = gstat                              # Conecting generating station into bus
        ...
        # -----------------------------------------
        # Adjusting plants with wind generation (in case of representation of renewables)
        if(self.simulation_set.RES_consid[0] == "True"):
                
            # Generation station on Bus 1 (2 x 76MW = 152MW --> 200 x 2.5MW = 500MW)
            self.system.dgstat[1].nu = 200
            self.system.dgstat[1].nu_available = self.system.dgstat[1].nu
            self.system.dgstat[1].P_min = 0.0
            self.system.dgstat[1].P_max = 2.5
            self.system.dgstat[1].P_max_orig = self.system.dgstat[1].P_max
            self.system.dgstat[1].failure_rate = 4.0
            self.system.dgstat[1].MTTR = 90.0
            self.system.dgstat[1].FOR = self.system.dgstat[1].failure_rate / (self.system.dgstat[1].failure_rate + (8760/self.system.dgstat[1].MTTR))

            # Generation station on Bus 15 (1 x 155MW = 155MW --> 200 x 2.5MW = 500MW)
            self.system.dgstat[7].nu = 200
            self.system.dgstat[7].nu_available = self.system.dgstat[7].nu
            self.system.dgstat[7].P_min = 0.0
            self.system.dgstat[7].P_max = 2.5
            self.system.dgstat[7].P_max_orig = self.system.dgstat[7].P_max
            self.system.dgstat[7].failure_rate = 4.0
            self.system.dgstat[7].MTTR = 90.0
            self.system.dgstat[7].FOR = self.system.dgstat[7].failure_rate / (self.system.dgstat[7].failure_rate + (8760/self.system.dgstat[7].MTTR))
    ...
    def load_input_data_AC(self, _inputDir, _output):
        ...
        # Opening file
        name_file = self.simulation_set.system_assessment[0][:-3] +'_'+self.simulation_set.region_assessment[0] +'_AC'+".dat"
        self._systemdata =  open(name_file,'r',encoding='utf-8')
        linesfile = self._systemdata.readlines()
    ...
    # -----------------------------------------
    # Method to load the renewable energy generation curve data    
    def load_RES_curve(self):

        # -----------------------------------------
        # Opening file and getting the values
        with open('IEEERTS_RES.res','r',encoding='utf-8') as file:
            cont_line = 0
            for line in file:
                if(cont_line > 0):
                    self.REScurve.append([float(line[0:8]),float(line[9:17])])
                cont_line += 1
...
class simulation_settings:
    ...
    def load_simulation_settings(self):
        ...
        self.region_assessment = linesfile[cont_line][20:28].split() # Region of assessment: "GLOBAL", "AREA", "BUS"
        self.loss_consid = linesfile[cont_line][29:35].split()       # Will ohmic losses be considered? (True or False)
        self.RES_consid = linesfile[cont_line][36:42].split()        # Will uncertainties related to renewable energy sources (RES) be considered? (True or False)
        if(self.loss_consid[0] == 'True'):
            self.iter = 2              # Two iterations will be performed to estimate the ohmic losses (Flow DC and OPF)
        ...
        self.reoptimization = linesfile[cont_line][38:44].split()    # Will reoptimization be used for OPFs? (True or False)
        ...
        while(linesfile[cont_line][0:4] != '####'):
            ...
            # Reward function weights
            self.c_LS.append(float(linesfile[cont_line][113:117]))                 # List with load shedding penalty weights
            self.c_cir.append(float(linesfile[cont_line][118:122]))                # List with line flow violation penalty weights 
            self.c_SW.append(float(linesfile[cont_line][123:127]))                 # List with SW generator limit violation penalty weights
            self.c_V.append(float(linesfile[cont_line][128:132]))                  # List with voltage magnitude violation penalty weights
            self.c_Q.append(float(linesfile[cont_line][133:137]))                  # List with reactive power generation violation penalty weights
            # GNNs settings
            self.hidden_dim_actor.append(int(linesfile[cont_line][138:143]))       # List with numbers of hidden dimensions for actor GNN
            self.n_heads_actor.append(int(linesfile[cont_line][144:148]))          # List with numbers of heads for actor GNN
            self.n_layers_actor.append(int(linesfile[cont_line][149:153]))         # List with numbers of layers for actor GNN
            self.hidden_dim_critic.append(int(linesfile[cont_line][154:159]))      # List with numbers of hidden dimensions for critic GNN
            self.n_heads_critic.append(int(linesfile[cont_line][160:164]))         # List with numbers of heads for critic GNN
            self.n_layers_critic.append(int(linesfile[cont_line][165:169]))        # List with numbers of layers for critic GNN
            self.std_min.append(float(linesfile[cont_line][170:176]))              # Minimal value for standart deviation (exploration parameter)
            self.std_max.append(float(linesfile[cont_line][177:183]))              # Maximal value for standart deviation (exploration parameter)
            self.dropout.append(float(linesfile[cont_line][184:190]))              # Dropout rate
            self.RA_crude_eval.append((linesfile[cont_line][191:197].split())[0])  # Will a crude MCS (OPF without optimizaion) be performed? (True or False)
            self.RA_GNN_PT_eval.append((linesfile[cont_line][198:204].split())[0]) # Will a GNN-MCS (trained only via SL - PT) be performed? (True or False)
            self.RA_GNN_RL_eval.append((linesfile[cont_line][205:211].split())[0]) # Will a GNN-MCS (trained via RL) be performed? (True or False)
            self.samples_performance.append(int(linesfile[cont_line][212:220]))    # Total of samples to performance analysis 
            cont_line += 1
