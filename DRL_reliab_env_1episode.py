...
class CompositeReliabilityEnv_1episode(gym.Env):
    ...
    def __init__(self, _data):
        ...
        self.c_LS = self.data.simulation_set.c_LS[cont_Test]     # Line flow violation penalty weight 
        self.c_cir = self.data.simulation_set.c_cir[cont_Test]   # Line flow violation penalty weight 
        self.c_SW = self.data.simulation_set.c_SW[cont_Test]     # SW generator limit violation penalty weight
        self.c_V = self.data.simulation_set.c_V[cont_Test]       # Voltage magnitude violation penalty weight
        self.c_Q= self.data.simulation_set.c_Q[cont_Test]        # Reactive power generation violation penalty weight
    ...
    def _compute_reward_pf(self, success):
        ...
        reward = 1.0 + (    self.c_LS * load_shed
                          + self.c_V * voltage_violation 
                          + self.c_cir * circuit_overload
                          + self.c_SW * PG_SW_violation 
                          + self.c_Q * QG_SW_PV_violation) / 1.0

        self.last_reward_info = {
                "load_shed":        self.c_LS  * load_shed,
                "voltage":          self.c_V   * voltage_violation,
                "circuit":          self.c_cir * circuit_overload,
                "sw_active":        self.c_SW  * PG_SW_violation,
                "reactive":         self.c_Q   * QG_SW_PV_violation,
            }
    ...
    def _compute_reward_opf(self, success):
        ...
        reward = (  self.c_LS * load_shed) / 1.0
