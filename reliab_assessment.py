...
class Reliab_assessment:
    ...
    def run_MCS_DRL_PPO_GNN_Lagrangian_AC(self, _data, _contTest, _mainDir, _outputDir, _modelDir):
        ...
        file_name = f'00_log_Lagrangian_Test_{_contTest + 1}.txt'     # distinct log
    ...
    # -----------------------------------------
    # Method to run Monte Carlo simulation - GNN model + OPF - AC (BATCHED INFERENCE)
    def run_MCS_GNN_OPF_AC_PROPOSAL(self, _data, _mainDir, _outputDir, _contTest, _modelDir, model_fase):
    ...
    # -----------------------------------------
    # Method to print results of Monte Carlo simulation - GNN - Buses level
    def print_results_MCS_GNN_OPF_AC_Buses_PROPOSAL(self, _data, _mainDir, _outputDir, _versionGNN):
        ...
        file_name = '{}_{}_MCS_GNN_{}.txt'.format(_data.simulation_set.current_test + 1, system, _versionGNN)
        ...
        file.write(' Region of Assessment: {}'.format(_data.simulation_set.region_assessment[0]))
        file.write('\n With loss? {}\n'.format(_data.simulation_set.loss_consid))
        file.write(' With reoptmization? {}\n'.format(_data.simulation_set.reoptimization))
        ...
        print(' Region of Assessment: {}'.format(_data.simulation_set.region_assessment[0]))
        print(' With loss? {}'.format(_data.simulation_set.loss_consid))
        print(' With reoptmization? {}'.format(_data.simulation_set.reoptimization))
        ...
        print(' Number of flowDCs:          {:9d}'.format(self.N_flowDC))  
            ...
            if not success:
                ...
                if(circuit_violation > 0.0 or voltage_violation > 0.0 or reactive_violation > 0.0 or activeSW_violation > 0.0 or island_load_shedding > 0.0):
                    ...
                    if(print_OPF_results):
                        OPF_AC_obj.print_OPF_AC(_data, _file_name='-OPF-'+str(cont_eval))
...
class System_state:
    ...
    # -----------------------------------------
    # Method to define state IF and IS
    def set_IF_IS(self):
        for g in range(0, len(self.unavailable_gstat['gstat'])):
            self.F_IF += self.unavailable_gstat['n_u'][g] * self.unavailable_gstat['gstat'][g].PsiF
            self.F_IS += self.unavailable_gstat['n_u'][g] * self.unavailable_gstat['gstat'][g].PsiS
        for c in range(0, len(self.unavailable_circ['circ'])):
            self.F_IF += self.unavailable_circ['n_u'][c] * self.unavailable_circ['circ'][c].PsiF
            self.F_IS += self.unavailable_circ['n_u'][c] * self.unavailable_circ['circ'][c].PsiS

    # -----------------------------------------
    # Method to print state IF and IS
    def print_IF_IS(self, _dgstat, _dcir, _mainDir, _outputDir):
        
        # Changing the directory 
        os.chdir(_outputDir)

        # Creating file
        file_name = 'element_Scores_Psi.txt'
        file =  open(file_name, 'w')

        file.write('-------------------------\n')
        file.write('GENERATING STATION SCORES\n')
        file.write('-------------------------\n')

        file.write('   {:6s}   {:6s}    {:12s} {:12s} \n'.format('STAT', 'BUS', 'PsiF','PsiS'))
        for gstat in _dgstat:
             file.write('{:6d}  {:6d}  {:11.4f}  {:11.4f}\n'.format(gstat.id, gstat.bus.number, gstat.PsiF, gstat.PsiS))

        file.write('\n---------------\n')
        file.write('CIRCUITS SCORES\n')
        file.write('---------------\n')

        file.write('{:6s}  {:6s}-{:6s}    {:12s} {:12s} \n'.format('CIRC','BUS-F', 'BUS-T', 'PsiF','PsiS'))
        for circ in _dcir:
             file.write('{:2d}  {:6d}-{:6d}  {:11.4f}  {:11.4f}\n'.format(circ.id, circ.bF.number, circ.bT.number, circ.PsiF, circ.PsiS))
    ...
    # -----------------------------------------
    # Method to define state IUk
    def set_IUk(self, _n_elements_clusters_g, _n_elements_clusters_c):

        self.F_IUgk = [0.0] * len(_n_elements_clusters_g)
        self.F_IUck = [0.0] * len(_n_elements_clusters_c)

        for g,gstat in enumerate(self.unavailable_gstat['gstat']):
            self.F_IUgk[gstat.cluster] += self.unavailable_gstat['n_u'][g] * (gstat.PsiF - gstat.PsiS)
        for c,circ in enumerate(self.unavailable_circ['circ']):
            self.F_IUck[circ.cluster] += self.unavailable_circ['n_u'][c] * (circ.PsiF - circ.PsiS)

    # -----------------------------------------
    # Method to define state IA
    def set_IA(self, _dgstat, _dcir):
        for gstat in _dgstat:
            self.F_IAg.append(gstat.nu - gstat.nu_available)
        for cir in _dcir:
            if(cir.available == False):
                self.F_IAc.append(1.0)
            else:
                self.F_IAc.append(0.0)
    ...
    def __init__(self, node_feat_dim, edge_feat_dim, hidden_dim=256):
        ...
        self.conv1 = GATv2Conv(node_feat_dim, hidden_dim, heads=4, concat=True, edge_dim=edge_feat_dim)
        # conv2 output = hidden_dim * 2 (512)
        self.conv2 = GATv2Conv(hidden_dim * 4 + node_feat_dim, hidden_dim, heads=2, concat=True, edge_dim=edge_feat_dim)
        
        self.norm1 = nn.LayerNorm(hidden_dim * 4)
        self.norm2 = nn.LayerNorm(hidden_dim * 2)

        # UPDATED: Input must be 1024 (hidden_dim * 4)
        # Because: concat(mean_pool(512), max_pool(512)) = 1024
    ...
    def forward(self, data):
        ...
        # Layer 1
        h1 = F.elu(self.norm1(self.conv1(x, edge_index, edge_attr)))
        
        # Layer 2 with Skip-connection
        h1_skip = torch.cat([h1, x], dim=-1)
        h2 = F.elu(self.norm2(self.conv2(h1_skip, edge_index, edge_attr)))

        # Pooling: Each is 512 units
        mean_pool = global_mean_pool(h2, batch)
        max_pool = global_max_pool(h2, batch)
        
        # Combined vector is 1024 units
...
class GATBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, n_heads, edge_dim, dropout=0.0, concat=True):
        ...
        self.act  = nn.ReLU() # Switched to ReLU to match initialization scaling
...
class GNNEncoder(nn.Module):
    ...
    def __init__(self, node_feat_dim, edge_feat_dim, hidden_dim, n_heads, n_layers, dropout=0.0):
        ...
        # Input node projection (Stabilizes highly variable AC-OPF features)
...
class GNN_ACTOR(nn.Module):
    ...
    def set_std_bounds(self, std_min: list, std_max: list):
        """Mutate per-group bounds in-place at runtime (no optimizer disruption).
        Hard floor of 1e-4 guards against log-prob numerical issues."""
        floor = 1e-4
