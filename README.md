...
│
├── checksums.txt                 # SHA-256 checksums for key input and configuration files
│
...
- **Corrective controls**: Corrective control actions for each evaluation state can be found in files following the naming conventions: 
	- `AC-OPF-y.txt` - AC-OPF benchmark solution obtained using PYPOWER's `runopf()` function; 
...
### Reproducing Tables II to V

Tables II to V in the paper are obtained directly from the training and evaluation results generated by `main.py`.
...
1. Select the corresponding test system and simulation settings in `InputData/00_SIMULATION_SETTINGS.set`.
2. Run `main.py`.
3. Use the generated output files in the corresponding `OutputData/Result-.../` directory:
